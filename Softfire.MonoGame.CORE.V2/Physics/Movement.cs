...
namespace Softfire.MonoGame.CORE.V2.Physics
{
    /// <summary>
    ...
    public class Movement : IMonoGameMovementComponent, IMonoGameUpdateComponent
    {
        ...
        /// </summary>
        private List<Easing> Easings { get; } = new List<Easing>();
        ...
        public bool AddEasing(Easing easing)
        {
            if (!Identities.ObjectExists<Easing, Easing>(Easings, easing.Name))
            {
                Easings.Add(easing);
                return true;
            }
        ...
        {
            if (Identities.ObjectExists<Easing, Easing>(Easings, name))
            {
                return Identities.GetObject<Easing, Easing>(Easings, name);
            }
        ...
        public bool RemoveEasing(string name)
        {
            if (Identities.ObjectExists<Easing, Easing>(Easings, name))
            {
                return Identities.RemoveObject<Easing, Easing>(Easings, name);
            }
